import { parseAliasFromSingleLine, HerbAlias } from "./herbAliasLoader";

type TestData = {
  source: string;
  expectedResult: HerbAlias;
};
describe("parseAliasFromSingleLine", () => {
  it("alias not null", () => {
    const testDataSet: TestData[] = [
      {
        source: "土三七：  藤三七，为落葵科植物落葵薯的块茎。处方别名：土三七 藤三七",
        expectedResult: {
          name: "土三七",
          alias: ["藤三七"]
        }
      ...
      {
        source: "混淆品：关白附：为毛茛科植物黄花乌头的块根。处方别名：关白附、竹节白附",
        expectedResult: {
          name: "关白附",
          alias: ["竹节白附"]
        }
      },
      {
        ...
        expectedResult: {
          name: "三七",
          alias: ["田七", "参三七", "汉三七", "旱三七", "田三七", "田漆", "滇七", "山漆", "金不换", "文山七"]
        }
      },
      {
    ...
    testDataSet.forEach(({ source, expectedResult }) => {
      var actualResult = parseAliasFromSingleLine(source);
  ...
  it("alias is null", () => {
    ...
    testDataSet.forEach(line => {
      var actualResult = parseAliasFromSingleLine(line);
